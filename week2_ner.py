...
Original file is located at
    https://colab.research.google.com/github/hse-aml/natural-language-processing/blob/master/week2/week2-NER.ipynb
...
def build_dict(tokens_or_tags, special_tokens):
    ...
    unique_tokens_or_tags = []
    for token_or_tag in tokens_or_tags:
      unique_tokens_or_tags = unique_tokens_or_tags + token_or_tag
      
    unique_tokens_or_tags = list(set(unique_tokens_or_tags) - set(special_tokens))
    idx2tok = special_tokens + unique_tokens_or_tags

    for index, element in enumerate(idx2tok):
      tok2idx[element] = index
...
def perform_optimization(self):
    ...
    grads = [a[0] for a in self.grads_and_vars]
    vars = [a[1] for a in self.grads_and_vars]
    clipped_grads = [tf.clip_by_norm(grad, clip_norm) for grad in grads]
    self.grads_and_vars = tuple(zip(clipped_grads, vars))
#     tf.clip_by_norm(grads, clip_norm)
    ######### YOUR CODE HERE #############
